...
public class RegExpURLAnalyze {
	...
	private AuthorizeProtected superior;
	...
	public List<AuthorizeType> check(String requestUrl) {
		...
		if (superior != null && superior.getUrlProtected() != null && superior.getUrlProtected().check(requestUrl)) {
			result.add(AuthorizeType.SUPERIOR);
		}
		return result;
	}
	...
	public List<AuthorizeType> check(String action, String method) {
		...
		if (admin != null && admin.getActionProtected() != null 
				&& admin.getActionProtected().check(action, method)) {
			result.add(AuthorizeType.ADMIN);
		}

		if (superior != null && superior.getActionProtected() != null
				&& superior.getActionProtected().check(action, method)) {
			result.add(AuthorizeType.SUPERIOR);
		}
		return result;
	}
	...
	public void setSuperior(AuthorizeProtected superior) {
