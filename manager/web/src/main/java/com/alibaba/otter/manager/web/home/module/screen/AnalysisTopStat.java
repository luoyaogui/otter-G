...
public class AnalysisTopStat {
    ...
    public void execute(HttpSession session, @Param("searchKey") String searchKey, @Param("topN") int topN, @Param("statTime") int minute,
        ...
        if(userInfo != null && userInfo.getAuthorizeType().isAdmin()){ //确定是否进行过滤
        	tops = delayStatService.listTopDelayStat(searchKey, userInfo.getId());
        } else {
        	tops = delayStatService.listTopDelayStat(searchKey);
        }
